###------------------------------------------------------------------
## hafley_jurisdictional_analysis.R
...
##
## This script is submitted as part of the application for the role of Applied Demographer
## at the Carl Vinson Institute of Government
...
atl_bg <- get_acs(geography = "block group", variables = acs_vars, state = "GA", 
...
atl_sf <- get_acs(geography = "block group", variables = "B19001_001", state = "GA",
                  county = metro_counties, year = 2018,
...
# remove missing data in ATL metro (if census tracts)
# atl_wide <- atl_wide %>%
...
muni_tenure <- aw_interpolate(munis2, tid = OBJECTID, source = atl_wide, sid = "GEOID",
                              weight = "sum", output = "sf", extensive = c("rntocc_sf_est","tot_sf_est","HousUnits_est", "Rce_BlkNH_est","TotPop_est","totTenure_est"))
...
muni_tenure <- muni_tenure %>%
  mutate(pct_ih = round(ihsfrs/tot_sf_est*100,1),
         pct_ihsfr = round(ihsfrs/rntocc_sf_est*100,1))
...
# before I make figures, if I wanted to export any of the tbls or sfs, that is possible. 
...
# map percent of SFRs owned by IH in each muni
tm_shape(counties_sf) +
  tm_borders(lwd = 1.5) +
  tm_shape(muni_tenure) +
  tm_polygons("pct_ihsfr", palette = 'viridis', breaks = c(0,.5,1.5,3.5,4.8,11.3), lwd = 0.1,
              title = "Invitation Homes share of\nsingle-family rentals") +
  tm_shape(rd) +
  tm_lines(col = "#BEBEBE", lwd = .5, alpha = .7) +
...
# tmap_mode('view')
# map percent of SFs owned by IH in each muni
# tm_shape(muni_tenure) +
...
munis2 %>%
  ...
  labs(x = NULL, y = "Total Number of Single-Family Rentals",
       title = "Municipal Level Ownership",
       subtitle = "Incorporated Areas Where Invitation Homes Owns More Than 50 SFRs") +
  geom_text(size = 2.1, position = position_stack(vjust = 0.5)) +
...
?scale_fill_manual
ggplot(ih_counties_long, aes(x = reorder(short_name,count), y = count, fill = var)) +
  geom_col(position = 'stack') +
  scale_fill_manual(values=c("#AADE55","#41912E"), 
                    labels = c("SFRs in Municipalities", "SFRs in Unincorporated Areas")) +
  labs(x = NULL, y = "Total Number of Single-Family Rentals",
       title = "County-Level Ownership",
       subtitle = "Rental Giant Concentrated in Unicorporated Areas in Most Counties ",
       fill = "") +
  # geom_text(size = 2.5, position = position_stack(vjust = 0.5)) +
  # theme(axis.text.x=element_text(angle=60, hjust=1)) +
  theme_ipsum(axis_text_size = 9) +
...
###----------------------------------------------------------------------------------------
# this section builds a map of only geolocated addresses in incorporated areas, styled by 
# the total number of properties within a municipality
###----------------------------------------------------------------------------------------
